	...
	<div class="content">
		...
		<section>
			<div class="bookSearchGroup">
				<form action="/bookLocation" method="post" name="searchForm">
			...
			<div class="bookListGroup">
				<div class="sectionTitle">위치별 관리</div>
				<table class="list">
					<tr>
						<th>도서위치</th>
						<th>장르</th>
					...
					<c:set var="loca" value="" />
					<c:forEach items="${list.bookList }" var="book">
						<tr>
							<!-- 위치가 동일하면 rowspan 값을 증가 -->
							<c:if test="${book.bookLocation eq loca }">
								<script>
									$('td').each(function(){
										if($(this).text()=='${loca}'){
											...
											$(this).attr('rowspan',Number(rownum)+1);
										}
									});
								</script>
							</c:if>
							<!-- 새로운 위치 출력 -->
							<c:if test="${book.bookLocation ne loca }">
								<td>${book.bookLocation }</td>
								<c:set var="loca" value="${book.bookLocation}" />
							</c:if>
